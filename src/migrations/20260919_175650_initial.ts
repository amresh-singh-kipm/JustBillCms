...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`pages_blocks_faq\` (
  	...
  	\`emit_schema\` integer DEFAULT true,
  	\`block_name\` text,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`pages\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  ...
  await db.run(sql`CREATE TABLE \`footer_groups_nav_items\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` text NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`link_type\` text DEFAULT 'reference',
  ...
  `)
  await db.run(sql`CREATE INDEX \`footer_groups_nav_items_order_idx\` ON \`footer_groups_nav_items\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`footer_groups_nav_items_parent_id_idx\` ON \`footer_groups_nav_items\` (\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`site_settings_organization_same_as\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`url\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`site_settings_organization_same_as_order_idx\` ON \`site_settings_organization_same_as\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`site_settings_organization_same_as_parent_id_idx\` ON \`site_settings_organization_same_as\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`site_settings\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`site_name\` text DEFAULT 'JustBill' NOT NULL,
  	\`default_title\` text DEFAULT 'GST Billing & Accounting Software for India | JustBill' NOT NULL,
  	\`default_description\` text DEFAULT 'Create GST invoices, e-invoices and e-way bills, track stock and payments, and get GSTR-ready reports. Start free with JustBill, no credit card needed.' NOT NULL,
  	\`blog_description\` text DEFAULT 'Practical guides on GST invoicing, e-invoicing, e-way bills, GST returns and running the accounts of a small Indian business.',
  	...
  	\`analytics_ga4_measurement_id\` text,
  	\`no_index_site\` integer DEFAULT false,
  	\`updated_at\` text,
  	\`created_at\` text,
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`footer_groups_nav_items\`;`)
  await db.run(sql`DROP TABLE \`footer_groups\`;`)
  await db.run(sql`DROP TABLE \`footer\`;`)
  await db.run(sql`DROP TABLE \`footer_rels\`;`)
  await db.run(sql`DROP TABLE \`site_settings_organization_same_as\`;`)
